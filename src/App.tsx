      ...
      <div className="evnH">
        ...
        <div className="reviews overflow-x-hidden w-full">
          <div className='maRq'>
            ...
            <div className='block whitespace-break-spaces mx-1 text-[.7em] px-4 rounded-md border border-zinc-800 font-1 shadow-2xl text-left w-50' key={review.id}>
              <div className="nM -mb-1 pt-1 text-[.7em] flex items-center">{review.name} ✦ <span className='inline-block'>{review.company}</span></div>
        ...
        <div className="reels">
          <div className="grid grid-cols-3 gap-4 w-150">
            {reels.map(reel => <div key={reel.id}><video src={reel.src} autoPlay muted className='rounded-md' preload="metadata" loop></video></div>)}
          </div>
        </div>
      </div>
    </div>
